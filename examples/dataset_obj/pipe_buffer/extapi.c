...
__attribute__((annotate("ALLOC_HEAP_RET"), annotate("AllocSize:Arg1")))
void *syscall_mmap(void *addr, unsigned long len, int prot, int flags, int fildes, long off)
{
    return NULL;
}
...
int syscall_fcntl(int fd, int cmd, unsigned long udata)
{
	return 0;
}
...
int syscall_ioctl(int fd, unsigned int cmd, unsigned long arg)
{
	return 0;
}
