...
Automatic exploit generation is a promising technique for vulnerability assessment, but automatic heap layout manipulation for kernel exploits remains an open problem and is necessary for kernel exploit and AEG techniques. The main challenges are automatic primitive extraction and layout manipulation under noisy primitives involving multiple objects. We present KeapMan, a system that combines iterative primitive extraction with planned adaptive tuning to facilitate automatic heap layout manipulation for Linux kernel exploits.
...
1. Compile and install SVF (and its companion LLVM) from source code, and download the kernel source code (currently using version 8357f6fb3d9a02ac55f0d758b9c79b4647c18bcb).
2. Prepare a file system for booting the kernel.
3. Configure the relevant paths in the config.py file in the source code directory.

You can install SVF in your Ubuntu 22.04 environment by following these steps：

```shell
sudo apt update -y
sudo apt install curl wget unzip build-essential vim git -y
...
1. repro.prog (optional): A PoC directly generated by the fuzzer. It needs to be converted to a C PoC before it can be analyzed by this system. This PoC is retained as evidence that the PoC used for analysis was indeed generated by fuzzing, and not artificially constructed through cheating.

2. repro.c: A C POC conforming to the Syzkaller generation format, containing the allocation and deallocation operations for the target object. If manually constructed, ensure the program is linear (without loops) and modify system calls to the form syscall(__NR_open, ...).

3. commit: The kernel version (commit ID) corresponding to this repro.c file.

4. config: The kernel configuration corresponding to this repro.c file.

5. extapi.c: This file models the syscalls required for the allocation/deallocation of this object, used to handle memory side effects during static analysis. For example, if one syscall writes a value to user memory, and another syscall needs to use that value, this modeling is needed to analyze the dependency between the two syscalls.

6. sites.json: format as follow:
...
7. If the primitives are not partitioned correctly, return to step c, increase syscall_num and try partitioning the primitives again. Generally, syscall_num is initially set to 1, and then each syscall is added up one by one. Repeat steps c-f until the correct allocation primitives are obtained.
...
You can configure PANDA by following these steps:


Then, you need to copy the input and sym directories in the examples directory to your working directory. Then, you need to put the compiled kernel bzImage and vmlinux into the input directory. We have prepared a workdir example in examples/work/.

Please refer to the fengshui.py file for specific input format.

## poc writing

Then put your original POC into the input directory, where you need to define two functions, begin and end. The content between the two functions is the heap feng shui part to be tested. Also pay attention to the compile.sh file in the input directory and modify the corresponding compilation commands to suit your POC.

Normally, memory operation sequences are measured in syscall units, but if you want to customize primitive units, you can define two functions, prim_in and prim_out, in your POC to mark the entry and exit of the primitive:
...
Where syscall_i can represent either a syscall or a primitive you define yourself. Numbers greater than or equal to 0 represent the relative positions within the slabs of each cache. While -1 represents selecting a random location for the noise.

Additionally, the snapshot process may cause issues with the terminal output, so you will need to use the reset command to restore it.